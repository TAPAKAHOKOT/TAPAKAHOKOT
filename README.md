...
```text
A new project to improve the logistics of recycling collection. It uses django (for back) + vue3 (for front)
```

[Tapakahokot VPN bot](https://t.me/tapakahokot_vpn_bot)
```text
Telegram bot to sell VPN (Trojan based on Xray-core). It uses aiogram3 (for tg bot) + flask (for webapp) + custom tg bot template.
